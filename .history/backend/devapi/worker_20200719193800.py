...
# API to add a list of subcategories of a given category
...
def create_subcat():
    if request.method == 'PUT':
        ...
        """
        req_data = request.get_json()
        # get the name of the main category form the input
        main_cat_name = req_data("mainCategoryName")
        main_cat_name = '{}'.format(main_cat_name)

        # get the sub category list from the input
        subCategoryList = req_data.get("subCategoryList")
        
        # Read request to get the categoryID of the given main category
        r0 = requests.post(
            url="http://3.129.56.173:6000/api/v1/db/read"
            json={
                "table":"main_categories",
                "columns": ["category_code"],
                "where":["category_name = " + main_cat_name]  
                # so, main_catrgories table me category_name is the name of the column
                # and we need the id of the column where the category_name is main_cat_name(given main category name)
                # ok peace,,,,
                }
            )
        ...
        if res["status"]==201:
            # Read the main category code from the response
            mainCategoryId = res["category_code"][0]

            # Count variable to make sure all subcategories were successfully written
            successCount = 0

            # Iterate through the list of subcategories to add a subcategory one at a time
            for subCategoryName in subCategoryList:
                ...
                if r1.json()["status"]==201:
                    # Increase the success cpunt if the write was successfull
                    successCount+=1

            if successCount == len(subCategoryList):
                return Response(status=201)
            else:
                result = {}
                result["status"] = 201
                result["message"] = "Some subcategories not inserted"
                return json.dumps(result)    
        else:
            return Response(status=401)
